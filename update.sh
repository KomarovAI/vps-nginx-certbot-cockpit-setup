...
log "Начало обновления VPS окружения для домена: $DOMAIN"
...
# Обновление времени на главной странице (если существует)
if [ -f "/var/www/${DOMAIN}/index.html" ]; then
    log "Обновление времени на сайте..."
    sed -i "s/Дата установки:.*/Обновлено: $(date '+%B %d, %Y, %H:%M MSK')/g" /var/www/${DOMAIN}/index.html
fi
