...
INSTALL_SCRIPT_URL="https://raw.githubusercontent.com/KomarovAI/vps-nginx-certbot-cockpit-setup/feat/custom-marzban-container/install.sh"
...
if [[ "$DEPLOY_MARZBAN" == "true" ]]; then
    ...
    echo "   Marzban Subdomain: https://marzban.$DOMAIN_NAME (if SSL available)"
    echo
    ...
    echo "   Update: /root/marzban-manage.sh update"
fi

echo
log "📊 Monitoring:"
echo "   Service check: /root/check-services.sh"
echo "   Real-time monitor: /root/monitor.sh"
echo "   Health check: /root/health-check.sh"
echo "   Backup configs: /root/backup-configs.sh"
