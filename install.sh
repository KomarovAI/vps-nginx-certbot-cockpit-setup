...
validate_environment(){
  ...
  if [[ "${DEPLOY_MARZBAN:-false}" == "true" ]]; then log INFO "Marzban: ENABLED port=${MARZBAN_PANEL_PORT:-8000} Xray=${XRAY_PORT:-2083}"; else log INFO "Marzban: DISABLED"; fi
}
...
if [[ -d "/opt/marzban-deployment/marzban" ]]; then cd /opt/marzban-deployment/marzban; if docker-compose ps 2>/dev/null | grep -q Up; then echo "Marzban: ✓ Running"; else echo "Marzban: ✗ Stopped"; fi; fi
EOF
